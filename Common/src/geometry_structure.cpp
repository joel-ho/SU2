...
void CGeometry::Gauss_Elimination(su2double** A, su2double* rhs, unsigned short nVar) {
  ...
  if (nVar == 1)
    ...
    for (iVar = 1; iVar < (short)nVar; iVar++) {
      for (jVar = 0; jVar < iVar; jVar++) {
        weight = A[iVar][jVar]/(A[jVar][jVar]+EPS);
