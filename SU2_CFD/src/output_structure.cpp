...
void COutput::SetForceSections(CSolver *solver_container, CGeometry *geometry,
  ...
  su2double *Plane_P0, *Plane_Normal, MinPlane, MaxPlane, *CPressure, MinXCoord,
  MaxXCoord, Force[3], ForceInviscid[3], MomentInviscid[3] =
  ...
  if (rank == MASTER_NODE) {
    ...
    cout << "Writing Cp sections and load distribution files." << endl;
  ...
  if (geometry->GetnDim() == 3) {
    ...
    for (iSection = 0; iSection < nSection; iSection++) {
      ...
      MinPlane = config->GetSection_VolumeBounds(0);
      MaxPlane = config->GetSection_VolumeBounds(1);
      MinXCoord = -1E6;
      MaxXCoord = 1E6;
      ...
      geometry->ComputeAirfoil_Section(Plane_P0, Plane_Normal, MinXCoord,
                                       MaxXCoord, CPressure, Xcoord_Airfoil, Ycoord_Airfoil, Zcoord_Airfoil,
                                       Pressure_Airfoil, true, config);
      
      if ((rank == MASTER_NODE) && (Xcoord_Airfoil.size() == 0)) {
          cout << "Please check the config file, the section " << Plane_P0[config->GetAxis_Orientation()]
      ...
      if ((rank == MASTER_NODE) && (Xcoord_Airfoil.size() != 0)) {
        ...
        if (config->GetSystemMeasurements() == SI) Cp_File << "ZONE T=\"y = " << Plane_P0[config->GetAxis_Orientation()] << " m\", I= "
          << Xcoord_Airfoil.size() << ", F=POINT" << "\n";;
        
        if (config->GetSystemMeasurements() == US) Cp_File << "ZONE T=\"y = " << Plane_P0[config->GetAxis_Orientation()]*12.0 << " in\", I= "
          << Xcoord_Airfoil.size() << ", F=POINT" << "\n";;
